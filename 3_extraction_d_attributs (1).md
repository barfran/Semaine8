...
imagesNB = images.apply(grayscaleImage)

pleines = load_images(dataset_dir, "a*.png")

croiss = load_images(dataset_dir, "b*.png")
```
...
def my_foreground_filter(img):
    foreground = foreground_redness_filter(img, theta=.55)
    foreground = invert_if_light_background(foreground)
    foreground2 = foreground_filter_white(img,80) #90
    ...
    if(a > b):
        ...
        return foreground
    #foreground2 = scipy.ndimage.gaussian_filter(foreground2, sigma=.1)
    return foreground2
    #return foreground
```

```{code-cell} ipython3
#img = transparent_background(imagesNB[0],foreground_filter2(imagesNB[0]))
foreground = foreground_filter2(imagesNB[0],theta =100)
imgp = transparent_background(imagesNB[0], foreground)
plt.imshow(imgp)
```
...
def my_preprocessing(img):
    ...
    foreground = foreground_filter2(img,theta = 45) #32
    foreground = scipy.ndimage.gaussian_filter(foreground, sigma=.12) #0.12
    img2 = transparent_background(img, foreground)
    #coordinates = np.argwhere(foreground)
...
```{code-cell} ipython3
plt.imshow(my_preprocessing(imagesNB[0]));
```
...
clean_imagesNB = imagesNB.apply(my_preprocessing)
...
for name, img in images.items():
    img.save(os.path.join('data1', name[:-4]+".png"))
```
...
df_features = pd.DataFrame({'redness': clean_images.apply(redness),
                            ...
                            'MatchedFilter': clean_imagesNB.apply(pleineOuPas,args =(pleines,croiss)),
                            'Moyenne Pixel': [MoyPix(img) for img in images]})
df_features
```
...
Pour les pleines lunes et les croissants de lunes, le matched filter et le perimetre ont des fortes elongations ( respectivement 0.73 et 0.65) avec les étiquettes. De plus on peut noter que l'élongation (l'attribut qui nous paraissait le plus approprié pour ce jeu de données) a une forte anti-corrélation (-0.69).  /// 1.3.2 (case exercice) Ici il est intéressant de prendre 5 attributs car sur la représentation graphique on voit que c'est lorsqu'on l'en prend 5, que ce soit le test ou le training, ils ont des dispersions faibles donc cela montre que lors des entrainements il y a eu peu de différences et que c'est en mettant 5 attributs que ce sera le plus stable. ///1.3.2 (case exercice 2) Meilleure paire pour lunes : Matched filter et elongation. Avec notre jeu de données l'approche multi-varié peut être intéressante car la meilleure paire est faite des deux arguments qui ont les meilleures corrélations.
...
Meilleure paire pour lunes : Matched filter et elongation. Avec notre jeu de données l'approche multi-varié peut être intéressante car la meilleure paire est faite des deux arguments qui ont les meilleures corrélations.
...
Ici il est intéressant de prendre 5 attributs car sur la représentation graphique on voit que c'est lorsqu'on l'en prend 5, que ce soit le test ou le training, ils ont des dispersions faibles donc cela montre que lors des entrainements il y a eu peu de
...
- Remarque : nous n'avons pas fait appel à la fonction crop depuis la feuille cependant on a redimensionné les photos de sorte à ce qu'elle fasse 32x32x3

- Remarque bis : lorsqu'on utilise le forground sur nos lunes l'efficacité des tests et entrainements sont équivalents avec les images brutes donc pour nous le foreground ne peut pas être uniforme.

- Difficulté majeure = les lunes n'ont pas toutes le même éclairage ce qui modifie le fond et ce qui empêche un traitement uniforme des lunes. De plus, les lunes n'ont pas toutes la même couleur ce qui empêche d'utiliser un filtre de couleur particulier (par exemple la blueness aurait eu un fort impact sur certaines de nos images mais non uniforme car parfois le ciel est bleu et d'en d'autres les lunes sont bleues).

- Détecter le centre de l'image n'est pas utile ici car en majorité, les pleines lunes et les croissants de lune ont un barycentre semblable donc cet attribut n'est pas intéressant

- Les attributs les plus utiles sont le matched filter qui a été très efficace pour le projet1 , c'est pour ça que nous le remettons en place, et les attributs en lien avec la taille ( surface, pertimetre, elongation)
