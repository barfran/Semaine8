...
##############################################################################
# Nouveaux attributs
def pleineOuPas(img : Image.Image,images1,images2):
    pleine = MatchedFilter(images1)
    pas = MatchedFilter(images2)
    
    match1 = pleine.match(img)
    match2 = pas.match(img)
    
    if(elongation(img)> 2.0 or match1 < match2):
        return -1.0* match2
    return match1

def grayscale(img : Image.Image) ->  np.array:
    array = np.array(img)
...
def MoyPix(img : Image.Image):
    imageNB = img.convert('L')
    arrNB = np.asarray(imageNB)
    moy = np.mean(arrNB)
    return moy

def foreground_filter2(
    img: Union[Image.Image, np.ndarray], theta: int = 10
) -> np.ndarray:
    """Create a black and white image outlining the foreground."""
    M = np.array(img)  # In case this is not yet a Numpy array
    G = np.min(M[:, :, 0:3], axis=2)
    F =   -1.0 * theta < M[:, :, 0:3]-M[0, 0, 0:3] < theta
...
def grayscaleImage(img : Image.Image) ->  Image.Image:
    ...
    r, g, b = array[:,:,0], array[:,:,1], array[:,:,2]
    gray = 0.2989 * r + 0.5870 * g + 0.1140 * b
    array[:,:,0], array[:,:,1], array[:,:,2] = gray, gray ,gray
    
    return Image.fromarray(array)

def foreground_filter2(
    img: Image.Image, theta: int = 10
) -> np.ndarray:
    """Create a black and white image outlining the foreground."""
    M = np.array(img)  # In case this is not yet a Numpy array
    moy = 0
